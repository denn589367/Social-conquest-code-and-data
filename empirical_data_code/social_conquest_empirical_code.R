...
#only include the model which dAICc<2
fm1<-phyloglm(social_system~among_std+within_std+str_std+dtr_std,newdata_benign,newtree_benign,method="logistic_MPLE",btol=30)
fm2<-phyloglm(social_system~among_std+dtr_std,newdata_benign,newtree_benign,method="logistic_MPLE",btol=30)
fm3<-phyloglm(social_system~among_std+str_std+dtr_std,newdata_benign,newtree_benign,method="logistic_MPLE",btol=30)
Cand.mods <- list("fm1" = fm1, "fm2" = fm2, "fm3" = fm3)
...
m <- define_model_set(
  null=c(),
  one=c(range_size~ss),
  .common=c(ss~pre_var_among,range_size~pre+pre_var_among+tmp+dtr+str+body_mass)
)
positions <- data.frame(
...
bb<-MuMIn::model.avg(dd)
summary(bb)
#N=1820, phylopath (check direct or indirect effect)
...
####################################
newdata_benign_ab<- subset(newdata_benign, !(abundance %in% NA))
newdata_non<- subset(newdata_global, 
                     !(Clade %in% c("Accipitridae","BC7","Furnaridae","M4","P10","P13P14P16","P17P18","Rallidae",
                                    "RallidaeHeliornithidae","S13","Alcedinidae","Apodidae","BC6","CC8","S12","Bucerotidae",
                                    "M6","P2","Psittacidae2","S2","CC4","CC5CC6","EurylaimidaePittidae",
                                    "M5","P20b","P21","Paleognathae","Picidae","S10","S3","S9")))
