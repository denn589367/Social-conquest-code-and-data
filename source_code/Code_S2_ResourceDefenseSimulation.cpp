...
public:
    ...
    void	UpdateResource		(double InitRes);
	...
	int		GetIndivAge		(int i){ return _Individual[i].GetAge();			}
	double	GetIndivCoopDeg	(int i){ return _Individual[i].GetCoopDeg();		}
...
private:
	...
	double	_Resource;		// total resource gain of the patch, equal to the fluctuating environmental resource availability
...
public:
	...
	// to handle floater dispersal (random dispersal)
	void	FloaterDisperse();
...
private:
	...
	Patch _FloaterPool;			// the place where the offspring will go if they do not sucessfully disperse to a patch
	vector<Patch> _Patch;
    ...
    double	_SurvRateUL;		// the upper limit of survival rate of the individuals in patches
	double	_FloaterSurvRateUL;	// the upper limit of survival rate of floaters
...
void PopProcess::SystemUpdate(int StartTime, int EndTime){
	for (int time = StartTime; time < EndTime; time++){
		...
		_FloaterPool.UpdateAge();
		_FloaterPool.UpdateIndivSurvRate(_FloaterSurvRateUL, _AgeStandard, _CostRate);
		_FloaterPool.UpdateAlive();
...
int main(int arc, char *argv[]){
	...
	while (cin >> res_mean >> res_half_var_range >> res_mean_offset >> res_half_var_range_offset >> coop_efficiency >> cost_rate >> init_coop_prop >> env_change_time \
        ...
        Simulation SUR(output_type, res_mean, res_half_var_range, res_mean_offset, res_half_var_range_offset, coop_efficiency, cost_rate, init_coop_prop, \
					   env_change_time, end_time, init_replic, replic_num, span, sociality);
	    SUR.ProcessShunt(output_type);
    }
...
void Patch::UpdateResource(double InitRes){
    _Resource = InitRes;
}

void Patch::UpdateIndivReproRate(double MetabolicConsump, double HalfConst, double ReproMax, double CostRate){
    // to compute individual intake and the resource-induced effect on individual reproduction rate
...
void Patch::UpdateIndivSurvRate(double SurvRateUL, double AgeStandard, double CostRate){
	for (Individual& indiv: _Individual){
		double indiv_surv_rate = SurvRateUL*exp(-indiv.GetAge()/AgeStandard);
		indiv.UpdateSurvRate(indiv_surv_rate);
	}
}
...
void PopProcess::FloaterDisperse(){
	for (int i = 0; i < _FloaterPool.GetSize(); i++){
		int patch_id = RG.SampleInt(0,_PatchNum);
		double access_prob = exp(-_Patch[patch_id].GetSize()*_CoopEfficiency*_Patch[patch_id].GetCoopDegSum()/_Patch[patch_id].GetResource());
		if (RG.RBern(access_prob)){
			_Patch[patch_id].AddIndividual(patch_id, _FloaterPool.GetIndivCoopDeg(i), _FloaterPool.GetIndivAge(i));
			_FloaterPool.EraseIndividual(i);
			i--;
		}
	}
}
	...
	for (int i = 0; i < _PatchNum; i++){
		...
		for (double parent_trait: offspring){
			...
			if (RG.RBern(access_prob)){
				...
				_FloaterPool.AddIndividual(-1, offspring_trait, 0);
			}
		}
	}
}
