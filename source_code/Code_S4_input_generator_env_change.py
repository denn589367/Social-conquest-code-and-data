...
def main():
    ...
    res_half_var_range = [[1, 4], [1, 10]]  # the half variation range of the environmental resource availability [[the half ranges used for simulations as res_mean == res_mean[0]], [the half ranges used for simulations as res_mean == res_mean[1]], ...]
    ...
    for s_type in sociality_type:
        data_path = f"data/env_change/{folders[s_type]}"
        ...
        for i, rm in enumerate(res_mean):
            for rhvr in res_half_var_range[i]:
                file_name_suffix = f"pfs_{rm:02}{rhvr:02}m0p0"
                input_file_name = f"{data_path}/{folders[s_type]}i_{file_name_suffix}.txt"
                output_file_name = f"{data_path}/{folders[s_type]}o_{file_name_suffix}.csv"
                ...
                for rmo in res_mean_offset:
                    file_name_suffix = f"pfs_{rm:02}{rhvr:02}m{abs(rmo)}00"
                    input_file_name = f"{data_path}/{folders[s_type]}i_{file_name_suffix}.txt"
                    output_file_name = f"{data_path}/{folders[s_type]}o_{file_name_suffix}.csv"
                    input_list.append(input_file_name)
                    output_list.append(output_file_name)
                ...
                for rhvro in res_half_var_range_offset:
                    file_name_suffix = f"pfs_{rm:02}{rhvr:02}00p{rhvro}"
                    input_file_name = f"{data_path}/{folders[s_type]}i_{file_name_suffix}.txt"
                    output_file_name = f"{data_path}/{folders[s_type]}o_{file_name_suffix}.csv"
                    input_list.append(input_file_name)
                    output_list.append(output_file_name)
        ...
        input_list_file_name = f"data/input_file_list_ec_{folders[s_type]}.txt"
        output_list_file_name = f"data/output_file_list_ec_{folders[s_type]}.txt"
